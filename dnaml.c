...
void   getoptions_ml(void);
void   allocrest_ml(void);
void   doinit_ml(void);
void   inputoptions(void);
void   makeweights_ml(void);
void   getinput(void);
void   inittable_for_usertree(FILE *);
void   inittable(void);
double evaluate_ml(node *, boolean);
...
longer seed;
Char* progname;
char basechar_ml[16]="acmgrsvtwyhkdbn";
...
double evaluate_ml(node *p, boolean saveit)
{
  contribarr tterm;
...
void rearrange_ml(node *p, node *pp)
{
...
void sigma(node *p, double *sumlr, double *s1, double *s2)
{
  ...
  p->v = epsilon;
  p->back->v = epsilon;
  aa = evaluate_ml(p, false);
  p->v = tt;
  p->back->v = tt;
  (*sumlr) = evaluate_ml(p, false) - aa;
  if (curv < -epsilon) {
...
void reconstr(node *p, long n)
{
  ...
  if ((ally[n] == 0) || (location[ally[n]-1] == 0))
    ...
    if (xx[first] < 0.4999995)
      m = m + (1 << second);
    if (xx[first] > 0.95)
      putc(toupper(basechar_ml[m - 1]), outfile);
    else
      putc(basechar_ml[m - 1], outfile);
    if (rctgry && rcategs > 1)
      mx = mp[n][mx - 1];    
    else
      mx = 1;
...
void summarize()
{
  ...
  describe_ml(curtree.start->back);
  ...
  dummy = evaluate_ml(curtree.start, false);
  if (rctgry && rcategs > 1) {
    for (i = 0; i < rcategs; i++)
      like[i] = 1.0;
    for (i = sites - 1; i >= 0; i--) {
...
void treevaluate_ml()
{
  ...
  dummy = evaluate_ml(curtree.start, true);
}  /* treevaluate */
    ...
    while (nextsp <= spp) {
      ...
      while (succeeded) {
        ...
        if (global && nextsp == spp)
          ...
          rearrange_ml(curtree.start, curtree.start->back);
        if (global && nextsp == spp && progress)
          putchar('\n');
      }
      nextsp++;
