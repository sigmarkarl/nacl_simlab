...
void *mymalloc2(size_t sz);       /* Prints "Out of memory" and exits on failure */
...
char buf2[100000];
char* erm(int argc, const char **argv) {
  ...
  for (iArg = 1; iArg < argc; iArg++) {
	...
	pseudoWeight = 1.0;
      ...
      iArg++;
      constraintsFile = (char*)argv[iArg];
  ...
  //postprint( "after args %d", iArg );
  //postprint( "after args2 %d", argc );
  ...
  char *fileName = iArg == (argc-1) ?  (char*)argv[argc-1] : NULL;
  ...
  int iAln;
  FILE *fpIn = fileName != NULL ? fmemopen(fileName, strlen(fileName), "r") : stdin;
  if (fpIn == NULL) {
  ...
  //postprint( "after readin" );
  //postprint( "in loop %d", (int)fpIn );
  //postprint( "in loop %d", (int)fileName );
  //postprint( "in loop %s", fileName );
  	  
  for(iAln = 0; iAln < nAlign; iAln++) {
	  alignment_t *aln = ReadAlignment(fpIn, bQuote);
    if (aln->nSeq < 1) {
      postprint("No alignment sequences\n");
      exit(1);
    }
    if (fpLog) {
      postprint("Read %d sequences, %d positions\n", aln->nSeq, aln->nPos);
      //fflush(fpLog);
    }
  ...
  if (verbose>1) {
	...
	tmatAsDist = (distance_matrix_t*)myfree(tmatAsDist, sizeof(distance_matrix_t));
	...
	if (MLlen) {
	  ...
	  for (iRound = 1; iRound <= maxRound; iRound++) {
	    int node;
	    float *oldlength = (float*)mymalloc2(sizeof(float)*NJ->maxnodes);
	    ...
	    oldlength = (float*)myfree(oldlength, sizeof(float)*NJ->maxnodes);
	...
	if (gammaLogLk && nRateCats > 1) {
	  ...
	  rates = (float*)myfree(rates, sizeof(float) * nRateCats);
	  site_loglk = (double*)myfree(site_loglk, sizeof(double) * nRateCats * NJ->nPos);
	...
	fflush(fp);
      ...
      FILE* mstdout = fmemopen( buf2, sizeof(buf2), "w" );
      PrintNJ(mstdout, NJ, aln->names, unique, /*support*/nBootstrap > 0, bQuote);
      fflush(mstdout);
      fclose(mstdout);
      if (fpLog) {
	...
	uniqConstraints = (char**)myfree(uniqConstraints, sizeof(char*) * unique->nUnique);
      constraints = FreeAlignment(constraints);
...
NJ_t *InitNJ(char **sequences, int nSeq, int nPos,
  ...
  NJ_t *NJ = (NJ_t*)mymalloc2(sizeof(NJ_t));
  ...
  NJ->profiles = (profile_t **)mymalloc2(sizeof(profile_t*) * NJ->maxnodes);
  ...
  NJ->diameter = (float *)mymalloc2(sizeof(float)*NJ->maxnodes);
  for (iNode = 0; iNode < NJ->maxnodes; iNode++) NJ->diameter[iNode] = 0;

  NJ->varDiameter = (float *)mymalloc2(sizeof(float)*NJ->maxnodes);
  for (iNode = 0; iNode < NJ->maxnodes; iNode++) NJ->varDiameter[iNode] = 0;

  NJ->selfdist = (float *)mymalloc2(sizeof(float)*NJ->maxnodes);
  for (iNode = 0; iNode < NJ->maxnodes; iNode++) NJ->selfdist[iNode] = 0;

  NJ->selfweight = (float *)mymalloc2(sizeof(float)*NJ->maxnodes);
  for (iNode = 0; iNode < NJ->nSeq; iNode++)
    NJ->selfweight[iNode] = NJ->nPos - NGaps(NJ,iNode);

  NJ->outDistances = (float *)mymalloc2(sizeof(float)*NJ->maxnodes);
  NJ->nOutDistActive = (int *)mymalloc2(sizeof(int)*NJ->maxnodes);
  ...
  NJ->parent = (int *)mymalloc2(sizeof(int)*NJ->maxnodes);
  for (iNode = 0; iNode < NJ->maxnodes; iNode++) NJ->parent[iNode] = -1;

  NJ->branchlength = (float *)mymalloc2(sizeof(float)*NJ->maxnodes); /* distance to parent */
  for (iNode = 0; iNode < NJ->maxnodes; iNode++) NJ->branchlength[iNode] = 0;

  NJ->support = (float *)mymalloc2(sizeof(float)*NJ->maxnodes);
  for (iNode = 0; iNode < NJ->maxnodes; iNode++) NJ->support[iNode] = -1.0;

  NJ->child = (children_t*)mymalloc2(sizeof(children_t)*NJ->maxnodes);
...
NJ_t *FreeNJ(NJ_t *NJ) {
  ...
  NJ->profiles = (profile_t**)myfree(NJ->profiles, sizeof(profile_t*) * NJ->maxnodes);
  NJ->outprofile = FreeProfile(NJ->outprofile, NJ->nPos, NJ->nConstraints);
  NJ->diameter = (float*)myfree(NJ->diameter, sizeof(float)*NJ->maxnodes);
  NJ->varDiameter = (float*)myfree(NJ->varDiameter, sizeof(float)*NJ->maxnodes);
  NJ->selfdist = (float*)myfree(NJ->selfdist, sizeof(float)*NJ->maxnodes);
  NJ->selfweight = (float*)myfree(NJ->selfweight, sizeof(float)*NJ->maxnodes);
  NJ->outDistances = (float*)myfree(NJ->outDistances, sizeof(float)*NJ->maxnodes);
  NJ->nOutDistActive = (int*)myfree(NJ->nOutDistActive, sizeof(int)*NJ->maxnodes);
  NJ->parent = (int*)myfree(NJ->parent, sizeof(int)*NJ->maxnodes);
  NJ->branchlength = (float*)myfree(NJ->branchlength, sizeof(float)*NJ->maxnodes);
  NJ->support = (float*)myfree(NJ->support, sizeof(float)*NJ->maxnodes);
  NJ->child = (children_t*)myfree(NJ->child, sizeof(children_t)*NJ->maxnodes);
  NJ->transmat = (transition_matrix_t*)myfree(NJ->transmat, sizeof(transition_matrix_t));
  AllocRateCategories(&NJ->rates, 0, NJ->nPos);
  return((NJ_t*)myfree(NJ, sizeof(NJ_t)));
}
...
void AllocRateCategories(/*IN/OUT*/rates_t *rates, int nRateCategories, int nPos) {
  ...
  rates->rates = (float*)myfree(rates->rates, sizeof(float)*rates->nRateCategories);
  rates->ratecat = (unsigned int*)myfree(rates->ratecat, sizeof(unsigned int)*nPos);
  rates->nRateCategories = nRateCategories;
  if (rates->nRateCategories > 0) {
    rates->rates = (float*)mymalloc2(sizeof(float)*rates->nRateCategories);
    int i;
    for (i = 0; i < nRateCategories; i++)
      rates->rates[i] = 1.0;
    rates->ratecat = (unsigned int *)mymalloc2(sizeof(unsigned int)*nPos);
    for (i = 0; i < nPos; i++)
      rates->ratecat[i] = 0;
  }
}
...
void FastNJ(NJ_t *NJ) {
  ...
  if (m>0) {
    ...
    visible = (besthit_t*)mymalloc2(sizeof(besthit_t)*NJ->maxnodes);
    besthitNew = (besthit_t*)mymalloc2(sizeof(besthit_t)*NJ->maxnodes);
    for (iNode = 0; iNode < NJ->nSeq; iNode++)
  ...
  for (nActive = NJ->nSeq; nActive > 3; nActive--) {
	...
	* (double)JoinConstraintPenalty(NJ, join.i, join.j);
	...
	&& changedActiveOutProfile >= fResetOutProfile * nActiveOutProfileReset) {
      ...
      profile_t **activeProfiles = (profile_t**)mymalloc2(sizeof(profile_t*)*(nActive-1));
      int nSaved = 0;
      NJ->totdiam = 0;
      for (iNode=0;iNode<NJ->maxnode;iNode++) {
	if (NJ->parent[iNode]<0) {
      ...
      NJ->outprofile = OutProfile(activeProfiles, nSaved,
				  NJ->nPos, NJ->nConstraints,
				  NJ->distance_matrix);
      activeProfiles = (profile_t**)myfree(activeProfiles, sizeof(profile_t*)*(nActive-1));
      nActiveOutProfileReset = nActive-1;
    } else {
  ...
  if (visible != NULL) visible = (besthit_t*)myfree(visible,sizeof(besthit_t)*NJ->maxnodes);
  if (besthitNew != NULL) besthitNew = (besthit_t*)myfree(besthitNew,sizeof(besthit_t)*NJ->maxnodes);
...
void ReadTree(/*IN/OUT*/NJ_t *NJ,
  ...
  */
  ...
  int *parent = (int*)mymalloc2(sizeof(int)*maxnodes);
  children_t *children = (children_t *)mymalloc2(sizeof(children_t)*maxnodes);
  int root = maxnode++;
  int i;
  ...
  int stack_size = 1;
  int *stack = (int*)mymalloc2(sizeof(int)*maxnodes);
  stack[0] = root;
  int nDown = 0;
  int nUp = 0;
  ...
  while ((token = ReadTreeToken(fpInTree)) != NULL) {
	...
	while (nDown-- > 0) {
	  int newc = maxnode++;
	  assert(newc < maxnodes);
	  ReadTreeAddChild(stack[stack_size-1], newc, /*IN/OUT*/parent, /*IN/OUT*/children);
	  if(verbose > 5)
	    fprintf(stderr, "Added internal child %d of %d, stack size increase to %d\n",
		    newc, stack[stack_size-1],stack_size+1);
	  stack[stack_size++] = newc;
  ...
  */
  ...
  int *map = (int*)mymalloc2(sizeof(int)*maxnodes);
  for (i = 0; i < unique->nUnique; i++)
    map[i] = i;
  ...
  map = (int*)myfree(map,sizeof(int)*maxnodes);
  stack = (int*)myfree(stack,sizeof(int)*maxnodes);
  children = (children_t*)myfree(children,sizeof(children_t)*maxnodes);
  parent = (int*)myfree(parent,sizeof(int)*maxnodes);
...
void PrintNJInternal(FILE *fp, NJ_t *NJ, bool useLen) {
  ...
  stack_t *stack = (stack_t *)mymalloc2(sizeof(stack_t)*NJ->maxnodes);
  int stackSize = 1;
  ...
  fprintf(fp, ";\n");
  stack = (stack_t*)myfree(stack, sizeof(stack_t)*NJ->maxnodes);
}
...
alignment_t *ReadAlignment(/*IN*/FILE *fp, bool bQuote) {
  ...
  if (fgets(buf,sizeof(buf),fp) == NULL) {
    postprint("Error reading header line");
    //exit(1);
  }
  int nSaved = 100;
  if (buf[0] == '>') {
    /* FASTA, truncate names at any of these */
    char *nameStop = (char*)(bQuote ? "'\t\r\n" : "(),: \t\r\n");
    char *seqSkip = " \t\r\n";	/* skip these characters in the sequence */
    seqs = (char**)mymalloc2(sizeof(char*) * nSaved);
    names = (char**)mymalloc2(sizeof(char*) * nSaved);
	...
	if (nSeq > nSaved) {
	  ...
	  seqs = (char**)myrealloc(seqs,sizeof(char*)*nSaved,sizeof(char*)*nNewSaved, /*copy*/false);
	  names = (char**)myrealloc(names,sizeof(char*)*nSaved,sizeof(char*)*nNewSaved, /*copy*/false);
	  nSaved = nNewSaved;
	}
	...
	*out = '\0';
    ...
    if (seqs[nSeq-1] == NULL) {
      postprint("No sequence data for last entry %s",names[nSeq-1]);
      //exit(1);
    }
    names = (char**)myrealloc(names,sizeof(char*)*nSaved,sizeof(char*)*nSeq, /*copy*/false);
    seqs = (char**)myrealloc(seqs,sizeof(char*)*nSaved,sizeof(char*)*nSeq, /*copy*/false);
  } else {
    ...
    if (buf[0] == '\n' || buf[0] == '\r') {
      if (fgets(buf,sizeof(buf),fp) == NULL) {
	postprint("Empty header line followed by EOF\n");
	//exit(1);
      }
    }
    ...
    names = (char **)mymalloc2(sizeof(char*) * nSeq);
    seqs = (char **)mymalloc2(sizeof(char*) * nSeq);
    nSaved = nSeq;

    int i;
    for (i = 0; i < nSeq; i++) {
      names[i] = NULL;
      seqs[i] = (char *)mymalloc2(nPos+1);	/* null-terminate */
      seqs[i][0] = '\0';
    }
    int iSeq = 0;
    ...
    if (iSeq != nSeq && iSeq != 0) {
      postprint("Wrong number of sequences: expected %d\n", nSeq);
    }
  }
  ...
  for (i = 0; i < nSeq; i++) {
    ...
    if (seqlen != nPos) {
      postprint("Wrong number of characters for");/* %s: expected %d but have %d instead.\n"
  ...
  if (findDot)
    postprint("Warning! Found \".\" character(s). These are treated as gaps\n");

  if (ferror(fp)) {
    postprint("Error reading input file\n");
    //exit(1);
  }

  alignment_t *align = (alignment_t*)mymalloc2(sizeof(alignment_t));
  align->nSeq = nSeq;
  align->nPos = nPos;
  align->names = names;
  align->seqs = seqs;
...
void FreeAlignmentSeqs(/*IN/OUT*/alignment_t *aln) {
  ...
  int i;
  for (i = 0; i < aln->nSeq; i++)
    aln->seqs[i] = (char*)myfree(aln->seqs[i], aln->nPos+1);
}
...
alignment_t *FreeAlignment(alignment_t *aln) {
  ...
  int i;
  for (i = 0; i < aln->nSeq; i++) {
    aln->names[i] = (char*)myfree(aln->names[i],strlen(aln->names[i])+1);
    aln->seqs[i] = (char*)myfree(aln->seqs[i], aln->nPos+1);
  }
  aln->names = (char**)myfree(aln->names, sizeof(char*)*aln->nSaved);
  aln->seqs = (char**)myfree(aln->seqs, sizeof(char*)*aln->nSaved);
...
char **AlnToConstraints(alignment_t *constraints, uniquify_t *unique, hashstrings_t *hashnames) {
  ...
  char **  uniqConstraints = (char**)mymalloc2(sizeof(char*) * unique->nUnique);	
  int i;
  for (i = 0; i < unique->nUnique; i++)
...
profile_t *AverageProfile(profile_t *profile1, profile_t *profile2,
  ...
  out->vectors = (float*)mymalloc2(sizeof(float)*nCodes*out->nVectors);
...
void UpdateOutProfile(/*IN/OUT*/profile_t *out, profile_t *old1, profile_t *old2,
  ...
  for (i = 0; i < nPos; i++) {
    ...
    float *fNew = GET_FREQ(newp,i,/*IN/OUT*/iFreqNew);
    ...
    double newMult = originalMult + newp->weights[i] - old1->weights[i] - old2->weights[i];
    ...
    if (newp->weights[i] > 0)
      AddToFreq(/*IN/OUT*/fOut, newp->weights[i], newp->codes[i], fNew, dmat);
  ...
  assert(iFreqNew == newp->nVectors);
  if(dmat)
    SetCodeDist(/*IN/OUT*/out,nPos,dmat);

  /* update constraints -- note in practice this should be a no-op */
  for (i = 0; i < nConstraints; i++) {
    out->nOn[i] += newp->nOn[i] - old1->nOn[i] - old2->nOn[i];
    out->nOff[i] += newp->nOff[i] - old1->nOff[i] - old2->nOff[i];
  }
}
...
void SetCodeDist(/*IN/OUT*/profile_t *profile, int nPos,
  ...
  if (profile->codeDist == NULL)
    profile->codeDist = (float*)mymalloc2(sizeof(float)*nPos*nCodes);
  int i;
  int iFreq = 0;
  for (i = 0; i < nPos; i++) {
...
distance_matrix_t *ReadDistanceMatrix(char *prefix) {
  ...
  distance_matrix_t *dmat = (distance_matrix_t*)mymalloc2(sizeof(distance_matrix_t));
...
profile_t *PosteriorProfile(profile_t *p1, profile_t *p2,
  ...
  if (transmat == NULL) {	/* Jukes-Cantor */
    ...
    PSame1 = (double*)myfree(PSame1, sizeof(double) * rates->nRateCategories);
    PSame2 = (double*)myfree(PSame2, sizeof(double) * rates->nRateCategories);
    PDiff1 = (double*)myfree(PDiff1, sizeof(double) * rates->nRateCategories);
    PDiff2 = (double*)myfree(PDiff2, sizeof(double) * rates->nRateCategories);
  ...
  if (transmat != NULL) {
    expeigenRates1 = (float*)myfree(expeigenRates1, sizeof(float) * rates->nRateCategories * nCodes);
    expeigenRates2 = (float*)myfree(expeigenRates2, sizeof(float) * rates->nRateCategories * nCodes);
  }
...
double *PSameVector(double length, rates_t *rates) {
  double *pSame = (double*)mymalloc2(sizeof(double) * rates->nRateCategories);
  int iRate;
  for (iRate = 0; iRate < rates->nRateCategories; iRate++)
...
double *PDiffVector(double *pSame, rates_t *rates) {
  double *pDiff = (double*)mymalloc2(sizeof(double) * rates->nRateCategories);
  int iRate;
  for (iRate = 0; iRate < rates->nRateCategories; iRate++)
...
float *ExpEigenRates(double length, transition_matrix_t *transmat, rates_t *rates) {
  float *expeigen = (float*)mymalloc2(sizeof(float) * nCodes * rates->nRateCategories);
  int iRate, j;
  for (iRate = 0; iRate < rates->nRateCategories; iRate++) {
    for (j = 0; j < nCodes; j++) {
...
double PairLogLk(profile_t *pA, profile_t *pB, double length, int nPos,
  ...
  if (transmat == NULL) {	/* Jukes-Cantor */
	...
	*/
    ...
    pSame = (double*)myfree(pSame, sizeof(double) * rates->nRateCategories);
    pDiff = (double*)myfree(pDiff, sizeof(double) * rates->nRateCategories);
  } else if (nCodes == 4) {	/* matrix model on nucleotides */
    int iFreqA = 0;
    int iFreqB = 0;
  ...
  if (transmat != NULL)
    expeigenRates = (float*)myfree(expeigenRates, sizeof(float) * rates->nRateCategories * 20);
  loglk += log(lk);
  nLkCompute++;
  return(loglk);
}
...
double TreeLogLk(/*IN*/NJ_t *NJ, /*OPTIONAL OUT*/double *site_loglk) {
  ...
  if (site_loglk != NULL) {
    site_likelihood = (double*)mymalloc2(sizeof(double)*NJ->nPos);
    for (i = 0; i < NJ->nPos; i++) {
  ...
  if (site_likelihood != NULL) {
    ...
    site_likelihood = (double*)myfree(site_likelihood, sizeof(double)*NJ->nPos);
  }
...
float *MLSiteRates(int nRateCategories) {
  ...
  float *rates = (float*)mymalloc2(sizeof(float)*nRateCategories);
  int i;
  for (i = 0; i < nRateCategories; i++)
...
double *MLSiteLikelihoodsByRate(/*IN*/NJ_t *NJ, /*IN*/float *rates, int nRateCategories) {
  double *site_loglk = (double*)mymalloc2(sizeof(double)*NJ->nPos*nRateCategories);

  /* save the original rates */
  assert(NJ->rates.nRateCategories > 0);
  float *oldRates = NJ->rates.rates;
  NJ->rates.rates = (float*)mymalloc2(sizeof(float) * NJ->rates.nRateCategories);
...
void SetMLRates(/*IN/OUT*/NJ_t *NJ, int nRateCategories) {
  ...
  */
  ...
  site_loglk = (double*)myfree(site_loglk, sizeof(double)*NJ->nPos*nRateCategories);
  ...
  /* Save the rates */
  NJ->rates.rates = (float*)myfree(NJ->rates.rates, sizeof(float) * NJ->rates.nRateCategories);
...
double GammaLogLk(/*IN*/siteratelk_t *s, /*OPTIONAL OUT*/double *gamma_loglk_sites) {
  int iRate, iPos;
  double *dRate = (double*)mymalloc2(sizeof(double) * s->nRateCats);
  ...
  dRate = (double*)myfree(dRate, sizeof(double)*s->nRateCats);
  return(loglk);
}
...
double RescaleGammaLogLk(int nPos, int nRateCats, /*IN*/float *rates, /*IN*/double *site_loglk,
  ...
  double *gamma_loglk_sites = (double*)mymalloc2(sizeof(double) * nPos);
  ...
  gamma_loglk_sites = (double*)myfree(gamma_loglk_sites, sizeof(double) * nPos);
  return(1.0/s.mult);
}
...
nni_stats_t *InitNNIStats(NJ_t *NJ) {
  nni_stats_t *stats = (nni_stats_t*)mymalloc2(sizeof(nni_stats_t)*NJ->maxnode);
...
nni_stats_t *FreeNNIStats(nni_stats_t *stats, NJ_t *NJ) {
  return((nni_stats_t*)myfree(stats, sizeof(nni_stats_t)*NJ->maxnode));
}
...
void SPR(/*IN/OUT*/NJ_t *NJ, int maxSPRLength, int iRound, int nRounds) {
  ...
  */
  ...
  int *nodeList = (int*)mymalloc2(sizeof(int) * NJ->maxnodes);
  int nodeListLen = 0;
  traversal_t traversal = InitTraversal(NJ);
  int node = NJ->root;
  while((node = TraversePostorder(node, NJ, /*IN/OUT*/traversal, /*pUp*/NULL)) >= 0) {
  ...
  profile_t **upProfiles = UpProfiles(NJ);
  spr_step_t *steps = (spr_step_t*)mymalloc2(sizeof(spr_step_t) * maxSPRLength); /* current chain of SPRs */
  ...
  steps = (spr_step_t*)myfree(steps, sizeof(spr_step_t) * maxSPRLength);
  upProfiles = FreeUpProfiles(upProfiles,NJ);
  nodeList = (int*)myfree(nodeList, sizeof(int) * NJ->maxnodes);
}
...
/* Pick columns for resampling, stored as returned_vector[iBoot*nPos + j] */
int *ResampleColumns(int nPos, int nBootstrap) {
  int *col = (int*)mymalloc2(sizeof(int)*nPos*nBootstrap);
  int i;
...
void ReliabilityNJ(/*IN/OUT*/NJ_t *NJ, int nBootstrap) {
  ...
  */
  ...
  traversal = FreeTraversal(traversal,NJ);
  upProfiles = FreeUpProfiles(upProfiles,NJ);
  col = (int*)myfree(col, sizeof(int)*NJ->nPos*nBootstrap);
}

profile_t *NewProfile(int nPos, int nConstraints) {
  profile_t *profile = (profile_t *)mymalloc2(sizeof(profile_t));
  profile->weights = (float*)mymalloc2(sizeof(float)*nPos);
  profile->codes = (unsigned char*)mymalloc2(sizeof(unsigned char)*nPos);
  ...
  if (nConstraints == 0) {
    ...
    profile->nOn = (int*)mymalloc2(sizeof(int)*nConstraints);
    profile->nOff = (int*)mymalloc2(sizeof(int)*nConstraints);
  }
  return(profile);
}
...
profile_t *FreeProfile(profile_t *profile, int nPos, int nConstraints) {
    ...
    return((profile_t*)myfree(profile, sizeof(profile_t)));
}
...
void TestSplitsML(/*IN/OUT*/NJ_t *NJ, /*OUT*/SplitCount_t *splitcount, int nBootstrap) {
  ...
  for (choice = 0; choice < 3; choice++)
    site_likelihoods[choice] = (double*)mymalloc2(sizeof(double)*NJ->nPos);
  ...
  if (nBootstrap>0)
    col = (int*)myfree(col, sizeof(int)*NJ->nPos*nBootstrap);
  for (choice = 0; choice < 3; choice++)
    site_likelihoods[choice] = (double*)myfree(site_likelihoods[choice], sizeof(double)*NJ->nPos);
}
...
double SplitSupport(profile_t *pA, profile_t *pB, profile_t *pC, profile_t *pD,
  ...
  double *weights[6];
  for (j = 0; j < 6; j++) {
    distpieces[j] = (double*)mymalloc2(sizeof(double)*nPos);
    weights[j] = (double*)mymalloc2(sizeof(double)*nPos);
  }
  ...
  */
  ...
  for (j = 0; j < 6; j++) {
    distpieces[j] = (double*)myfree(distpieces[j], sizeof(double)*nPos);
    weights[j] = (double*)myfree(weights[j], sizeof(double)*nPos);
  }
...
double SHSupport(int nPos, int nBootstrap, int *col, double loglk[3], double *site_likelihoods[3]) {
  ...
  int i,j;
  for (i = 0; i < 3; i++) {
    siteloglk[i] = (double*)mymalloc2(sizeof(double)*nPos);
    for (j = 0; j < nPos; j++)
  ...
  for (i=0;i<3;i++)
    siteloglk[i] = (double*)myfree(siteloglk[i], sizeof(double)*nPos);
  return(nSupport/(double)nBootstrap);
}
...
top_hits_t *FreeTopHits(top_hits_t *tophits) {
  ...
  for (iNode = 0; iNode < tophits->maxnodes; iNode++) {
    ...
    if (l->hits != NULL)
      l->hits = (hit_t*)myfree(l->hits, sizeof(hit_t) * l->nHits);
  }
  tophits->top_hits_lists = (top_hits_list_t*)myfree(tophits->top_hits_lists, sizeof(top_hits_list_t) * tophits->maxnodes);
  tophits->visible = (hit_t*)myfree(tophits->visible, sizeof(hit_t*) * tophits->maxnodes);
  tophits->topvisible = (int*)myfree(tophits->topvisible, sizeof(int) * tophits->nTopVisible);
#ifdef OPENMP
  for (iNode = 0; iNode < tophits->maxnodes; iNode++)
    omp_destroy_lock(&tophits->locks[iNode]);
  tophits->locks = myfree(tophits->locks, sizeof(omp_lock_t) * tophits->maxnodes);
#endif
  return((top_hits_t*)myfree(tophits, sizeof(top_hits_t)));
}

top_hits_t *InitTopHits(NJ_t *NJ, int m) {
  int iNode;
  assert(m > 0);
  top_hits_t *tophits = (top_hits_t*)mymalloc2(sizeof(top_hits_t));
  tophits->m = m;
  tophits->q = (int)(0.5 + tophits2Mult * sqrt(tophits->m));
  if (!useTopHits2nd || tophits->q >= tophits->m)
    tophits->q = 0;
  tophits->maxnodes = NJ->maxnodes;
  tophits->top_hits_lists = (top_hits_list_t*)mymalloc2(sizeof(top_hits_list_t) * tophits->maxnodes);
  tophits->visible = (hit_t*)mymalloc2(sizeof(hit_t) * tophits->maxnodes);
  tophits->nTopVisible = (int)(0.5 + topvisibleMult*m);
  tophits->topvisible = (int*)mymalloc2(sizeof(int) * tophits->nTopVisible);
#ifdef OPENMP
  tophits->locks = mymalloc2(sizeof(omp_lock_t) * tophits->maxnodes);
  for (iNode = 0; iNode < tophits->maxnodes; iNode++)
...
void SetAllLeafTopHits(/*IN/UPDATE*/NJ_t *NJ, /*IN/OUT*/top_hits_t *tophits) {
  ...
  */
  int *nGaps = (int*)mymalloc2(sizeof(int)*NJ->nSeq);
  int iNode;
  for(iNode=0; iNode<NJ->nSeq; iNode++) {
    nGaps[iNode] = (int)(0.5 + NJ->nPos - NJ->selfweight[iNode]);
  }
  int *seeds = (int*)mymalloc2(sizeof(int)*NJ->nSeq);
  ...
  for(iSeed=0; iSeed < NJ->nSeq; iSeed++) {
    ...
    besthit_t *besthitsSeed = (besthit_t*)mymalloc2(sizeof(besthit_t)*NJ->nSeq);
    besthit_t *besthitsNeighbor = (besthit_t*)mymalloc2(sizeof(besthit_t) * 2 * tophits->m);
    besthit_t bestjoin;
    ...
    */
	...
	besthit_t *besthitsClose = (besthit_t*)mymalloc2(sizeof(besthit_t) * nUse);
	...
	besthitsClose = (besthit_t*)myfree(besthitsClose, sizeof(besthit_t) * nUse);
	...
	for (iClose2 = 0; iClose2 < tophits->q && iClose2 < 2 * tophits->m; iClose2++) {
	  ...
	  if (tophits->top_hits_lists[closeNode2].hits == NULL) {
	    ...
	    nHasTopHits++;
	    int nUse = MIN(tophits->q * tophits2Safety, 2 * tophits->m);
	    besthit_t *besthitsClose2 = (besthit_t*)mymalloc2(sizeof(besthit_t) * nUse);
	    TransferBestHits(NJ, /*nActive*/NJ->nSeq,
			     closeNode2,
	    ...
	    besthitsClose2 = (besthit_t*)myfree(besthitsClose2, sizeof(besthit_t) * nUse);
	  } /* end if should do 2nd-level transfer */
	}
      }
    } /* end loop over close candidates */
    besthitsSeed = (besthit_t*)myfree(besthitsSeed, sizeof(besthit_t)*NJ->nSeq);
    besthitsNeighbor = (besthit_t*)myfree(besthitsNeighbor, sizeof(besthit_t) * 2 * tophits->m);
  } /* end loop over seeds */
  ...
  nGaps = (int*)myfree(nGaps, sizeof(int)*NJ->nSeq);
  seeds = (int*)myfree(seeds, sizeof(int)*NJ->nSeq);
...
besthit_t *UniqueBestHits(/*IN/UPDATE*/NJ_t *NJ, int nActive,
  ...
  besthit_t *uniqueList = (besthit_t*)mymalloc2(sizeof(besthit_t)*nCombined);
  int nUnique = 0;
  int iSavedLast = -1;
...
void TopHitJoin(int newnode,
  ...
  besthit_t *combinedList = (besthit_t*)mymalloc2(sizeof(besthit_t)*nCombined);
  ...
  combinedList = (besthit_t*)myfree(combinedList, sizeof(besthit_t)*nCombined);

  /* Forget the top-hit lists of the joined nodes */
  for (i = 0; i < 2; i++) {
    lChild[i]->hits = (hit_t*)myfree(lChild[i]->hits, sizeof(hit_t) * lChild[i]->nHits);
    lChild[i]->nHits = 0;
  }
  ...
  */
    ...
    */
	...
	&& tophits->top_hits_lists[source].hitSource < 0) {
	 ...
	 of node from what we have so far plus the active source plus its top hits */
      ...
      besthit_t *mergeList = (besthit_t*)mymalloc2(sizeof(besthit_t) * nMerge);
	...
	iMerge++;
      ...
      uniqueList = (besthit_t*)myfree(uniqueList, nUniqueAlloc * sizeof(besthit_t));
      uniqueList = UniqueBestHits(/*IN/UPDATE*/NJ, nActive,
				  /*IN/SORT*/mergeList,
				  nMerge,
				  /*OUT*/&nUnique);
      nUniqueAlloc = nMerge;
      mergeList = (besthit_t*)myfree(mergeList, sizeof(besthit_t)*nMerge);
  ...
  if (bUseUnique) {
    ...
    */
    ...
    besthit_t *allhits = (besthit_t*)mymalloc2(sizeof(besthit_t)*NJ->maxnode);
    ...
    for (iHit=0; iHit < tophits->m; iHit++) {
      ...
      besthit_t *bothList = (besthit_t*)mymalloc2(sizeof(besthit_t) * 3 * tophits->m);
	...
	l->hitSource = -1;	/* abandon the 2nd-level top-hits heuristic */
      ...
      besthit_t *uniqueList2 = UniqueBestHits(/*IN/UPDATE*/NJ, nActive,
					      ...
					      /*OUT*/&nUnique2);
      assert(nUnique2 > 0);
      bothList = (besthit_t*)myfree(bothList,3 * tophits->m * sizeof(besthit_t));
      ...
      uniqueList2 = (besthit_t*)myfree(uniqueList2, (nHitsOld + 2 * tophits->m) * sizeof(besthit_t));
    }

    ResetTopVisible(/*IN/UPDATE*/NJ, nActive, /*IN/OUT*/tophits); /* outside of the parallel phase */
    allhits = (besthit_t*)myfree(allhits,sizeof(besthit_t)*NJ->maxnode);
  }
  uniqueList = (besthit_t*)myfree(uniqueList, nUniqueAlloc * sizeof(besthit_t));
  if (verbose > 2) {
...
void ResetTopVisible(/*IN/UPDATE*/NJ_t *NJ,
		     int nActive,
		     /*IN/OUT*/top_hits_t *tophits) {
  besthit_t *visibleSorted = (besthit_t*)mymalloc2(sizeof(besthit_t)*nActive);
   ...
   */
  int *inTopVisible = (int*)malloc(sizeof(int) * NJ->maxnodes);
  int i;
  for (i = 0; i < NJ->maxnodes; i++)
    inTopVisible[i] = -1;
...
#endif
  if (l->hits != NULL) {
    l->hits = (hit_t*)myfree(l->hits, l->nHits * sizeof(hit_t));
    l->nHits = 0;
  }
  l->hits = (hit_t*)mymalloc2(sizeof(hit_t) * nSave);
  l->nHits = nSave;
  int iSave = 0;
  jLast = -1;
...
void TransferBestHits(/*IN/UPDATE*/NJ_t *NJ,
  ...
  for(iBest = 0; iBest < nOldHits; iBest++) {
    ...
    besthit_t *newb = &newhits[iBest];
    newb->i = iNode;
    newb->j = ActiveAncestor(/*IN*/NJ, old->j);
    newb->dist = old->dist;	/* may get reset below */
    newb->weight = old->weight;
    newb->criterion = old->criterion;

    if(newb->j < 0 || newb->j == iNode) {
      newb->weight = 0;
      newb->dist = -1e20;
      newb->criterion = 1e20;
    } else if (newb->i != old->i || newb->j != old->j) {
      if (updateDistances)
	SetDistCriterion(/*IN/UPDATE*/NJ, nActive, /*IN/OUT*/newb);
      else {
	newb->dist = -1e20;
	newb->criterion = 1e20;
      }
      ...
      if (updateDistances)
	SetCriterion(/*IN/UPDATE*/NJ, nActive, /*IN/OUT*/newb);
      else
	newb->criterion = 1e20;	/* leave dist alone */
    }
  }
}
...
void *mymalloc2(size_t sz) {
  ...
  assert(IS_ALIGNED(newp));
  return (newp);
}

void *mymemdup(void *data, size_t sz) {
  if(data==NULL) return(NULL);
  void *newp = mymalloc2(sz);
  memcpy(/*to*/newp, /*from*/data, sz);
  return(newp);
}

void *myrealloc(void *data, size_t szOld, size_t szNew, bool bCopy) {
  if (data == NULL && szOld == 0)
    return(mymalloc2(szNew));
  ...
  if (bCopy) {
    ...
    newp = mymemdup(data, szNew);
    myfree(data, szOld);
  } else {
    newp = realloc(data,szNew);
    if (newp == NULL) {
      fprintf(stderr, "Out of memory\n");
      exit(1);
    }
...
hashstrings_t *MakeHashtable(char **strings, int nStrings) {
  hashstrings_t *hash = (hashstrings_t*)mymalloc2(sizeof(hashstrings_t));
  hash->nBuckets = 8*nStrings;
  hash->buckets = (hashbucket_t*)mymalloc2(sizeof(hashbucket_t) * hash->nBuckets);
  int i;
...
uniquify_t *UniquifyAln(alignment_t *aln) {
    int nUniqueSeq = 0;
    char **uniqueSeq = (char**)mymalloc2(aln->nSeq * sizeof(char*)); /* iUnique -> seq */
    int *uniqueFirst = (int*)mymalloc2(aln->nSeq * sizeof(int)); /* iUnique -> iFirst in aln */
    int *alnNext = (int*)mymalloc2(aln->nSeq * sizeof(int)); /* i in aln -> next, or -1 */
    int *alnToUniq = (int*)mymalloc2(aln->nSeq * sizeof(int)); /* i in aln -> iUnique; many -> -1 */
    ...
    uniquify_t *uniquify = (uniquify_t*)mymalloc2(sizeof(uniquify_t));
...
traversal_t InitTraversal(NJ_t *NJ) {
  traversal_t worked = (bool*)mymalloc2(sizeof(bool)*NJ->maxnodes);
  int i;
  for (i=0; i<NJ->maxnodes; i++)
    worked[i] = false;
  return(worked);
...
profile_t **UpProfiles(NJ_t *NJ) {
  profile_t **upProfiles = (profile_t**)mymalloc2(sizeof(profile_t*)*NJ->maxnodes);
  int i;
...
int *PathToRoot(NJ_t *NJ, int node, /*OUT*/int *outDepth) {
  int *pathToRoot = (int*)mymalloc2(sizeof(int)*NJ->maxnodes);
  int depth = 0;
  int ancestor = node;
  while(ancestor >= 0) {
...
transition_matrix_t *CreateTransitionMatrix(/*IN*/double matrix[MAXCODES][MAXCODES],
					    /*IN*/double stat[MAXCODES]) {
  int i,j,k;
  transition_matrix_t *transmat = (transition_matrix_t*)mymalloc2(sizeof(transition_matrix_t));
  double sqrtstat[20];
  for (i = 0; i < nCodes; i++) {
