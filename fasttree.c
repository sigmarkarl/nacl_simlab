...
double *PSameVector(double length, rates_t *rates) {
  double *pSame = mymalloc2(sizeof(double) * rates->nRateCategories);
  int iRate;
  for (iRate = 0; iRate < rates->nRateCategories; iRate++)
...
double *PDiffVector(double *pSame, rates_t *rates) {
  double *pDiff = mymalloc2(sizeof(double) * rates->nRateCategories);
  int iRate;
  for (iRate = 0; iRate < rates->nRateCategories; iRate++)
...
float *ExpEigenRates(double length, transition_matrix_t *transmat, rates_t *rates) {
  float *expeigen = mymalloc2(sizeof(float) * nCodes * rates->nRateCategories);
  int iRate, j;
  for (iRate = 0; iRate < rates->nRateCategories; iRate++) {
    for (j = 0; j < nCodes; j++) {
...
double TreeLogLk(/*IN*/NJ_t *NJ, /*OPTIONAL OUT*/double *site_loglk) {
  ...
  if (site_loglk != NULL) {
    site_likelihood = mymalloc2(sizeof(double)*NJ->nPos);
    for (i = 0; i < NJ->nPos; i++) {
...
float *MLSiteRates(int nRateCategories) {
  ...
  float *rates = mymalloc2(sizeof(float)*nRateCategories);
  int i;
  for (i = 0; i < nRateCategories; i++)
...
double *MLSiteLikelihoodsByRate(/*IN*/NJ_t *NJ, /*IN*/float *rates, int nRateCategories) {
  double *site_loglk = mymalloc2(sizeof(double)*NJ->nPos*nRateCategories);

  /* save the original rates */
  assert(NJ->rates.nRateCategories > 0);
  float *oldRates = NJ->rates.rates;
  NJ->rates.rates = mymalloc2(sizeof(float) * NJ->rates.nRateCategories);
...
double GammaLogLk(/*IN*/siteratelk_t *s, /*OPTIONAL OUT*/double *gamma_loglk_sites) {
  int iRate, iPos;
  double *dRate = mymalloc2(sizeof(double) * s->nRateCats);
...
double RescaleGammaLogLk(int nPos, int nRateCats, /*IN*/float *rates, /*IN*/double *site_loglk,
  ...
  double *gamma_loglk_sites = mymalloc2(sizeof(double) * nPos);
...
nni_stats_t *InitNNIStats(NJ_t *NJ) {
  nni_stats_t *stats = mymalloc2(sizeof(nni_stats_t)*NJ->maxnode);
...
void SPR(/*IN/OUT*/NJ_t *NJ, int maxSPRLength, int iRound, int nRounds) {
  ...
  */
  ...
  int *nodeList = mymalloc2(sizeof(int) * NJ->maxnodes);
  int nodeListLen = 0;
  traversal_t traversal = InitTraversal(NJ);
  int node = NJ->root;
  while((node = TraversePostorder(node, NJ, /*IN/OUT*/traversal, /*pUp*/NULL)) >= 0) {
  ...
  profile_t **upProfiles = UpProfiles(NJ);
  spr_step_t *steps = mymalloc2(sizeof(spr_step_t) * maxSPRLength); /* current chain of SPRs */
...
profile_t *NewProfile(int nPos, int nConstraints) {
  ...
  profile->weights = mymalloc2(sizeof(float)*nPos);
  profile->codes = mymalloc2(sizeof(unsigned char)*nPos);
  ...
  if (nConstraints == 0) {
    ...
    profile->nOn = mymalloc2(sizeof(int)*nConstraints);
    profile->nOff = mymalloc2(sizeof(int)*nConstraints);
  }
  return(profile);
}
...
void TestSplitsML(/*IN/OUT*/NJ_t *NJ, /*OUT*/SplitCount_t *splitcount, int nBootstrap) {
  ...
  for (choice = 0; choice < 3; choice++)
    site_likelihoods[choice] = mymalloc2(sizeof(double)*NJ->nPos);
...
double SHSupport(int nPos, int nBootstrap, int *col, double loglk[3], double *site_likelihoods[3]) {
  ...
  int i,j;
  for (i = 0; i < 3; i++) {
    siteloglk[i] = mymalloc2(sizeof(double)*nPos);
    for (j = 0; j < nPos; j++)
...
top_hits_t *InitTopHits(NJ_t *NJ, int m) {
  int iNode;
  assert(m > 0);
  top_hits_t *tophits = mymalloc2(sizeof(top_hits_t));
  tophits->m = m;
  tophits->q = (int)(0.5 + tophits2Mult * sqrt(tophits->m));
  if (!useTopHits2nd || tophits->q >= tophits->m)
    tophits->q = 0;
  tophits->maxnodes = NJ->maxnodes;
  tophits->top_hits_lists = mymalloc2(sizeof(top_hits_list_t) * tophits->maxnodes);
  tophits->visible = mymalloc2(sizeof(hit_t) * tophits->maxnodes);
  tophits->nTopVisible = (int)(0.5 + topvisibleMult*m);
  tophits->topvisible = mymalloc2(sizeof(int) * tophits->nTopVisible);
#ifdef OPENMP
  ...
  */
  ...
  for(iSeed=0; iSeed < NJ->nSeq; iSeed++) {
    ...
    */
	...
	besthit_t *besthitsClose = mymalloc2(sizeof(besthit_t) * nUse);
	...
	for (iClose2 = 0; iClose2 < tophits->q && iClose2 < 2 * tophits->m; iClose2++) {
	  ...
	  if (tophits->top_hits_lists[closeNode2].hits == NULL) {
	    ...
	    nHasTopHits++;
	    int nUse = MIN(tophits->q * tophits2Safety, 2 * tophits->m);
	    besthit_t *besthitsClose2 = mymalloc2(sizeof(besthit_t) * nUse);
	    TransferBestHits(NJ, /*nActive*/NJ->nSeq,
			     closeNode2,
    ...
    */
	...
	&& tophits->top_hits_lists[source].hitSource < 0) {
	 ...
	 of node from what we have so far plus the active source plus its top hits */
      ...
      besthit_t *mergeList = mymalloc2(sizeof(besthit_t) * nMerge);
...
void ResetTopVisible(/*IN/UPDATE*/NJ_t *NJ,
		     int nActive,
		     /*IN/OUT*/top_hits_t *tophits) {
  besthit_t *visibleSorted = mymalloc2(sizeof(besthit_t)*nActive);
...
void SortSaveBestHits(int iNode, /*IN/SORT*/besthit_t *besthits,
  ...
  l->hits = mymalloc2(sizeof(hit_t) * nSave);
  l->nHits = nSave;
  int iSave = 0;
  jLast = -1;
...
transition_matrix_t *CreateTransitionMatrix(/*IN*/double matrix[MAXCODES][MAXCODES],
					    /*IN*/double stat[MAXCODES]) {
  int i,j,k;
  transition_matrix_t *transmat = mymalloc2(sizeof(transition_matrix_t));
  double sqrtstat[20];
  for (i = 0; i < nCodes; i++) {
...
distance_matrix_t *TransMatToDistanceMat(transition_matrix_t *transmat) {
  if (transmat == NULL)
    return(NULL);
  distance_matrix_t *dmat = mymalloc2(sizeof(distance_matrix_t));
  int i, j;
  for (i=0; i<nCodes; i++) {
    for (j=0; j<nCodes; j++) {
